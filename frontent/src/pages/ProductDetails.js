...
const ProductDetails = () => {
  ...
  const fechProductsDetails = async () => {
    setLoading(true);
    try {
      ...
      setData(dataResponse?.data || {}); // Ensure data is an object
      setActiveImage(dataResponse?.data?.productImage[0] || ""); // Set default active image
    } catch (error) {
      console.error(error);
      ...
      <div className='min-h-[200px] flex flex-col lg:flex-row gap-4'>
        ...
        {loading ? (
          ...
          <div className='flex flex-col gap-1'>
            <p className='bg-red-200 text-red-600 px-2 rounded-full inline-block w-fit'>{data?.brandName || "Brand Name"}</p>
            <h2 className='text-2xl lg:text-4xl font-medium'>{data?.productName || "Product Name"}</h2>
            <p className='capitalize text-slate-400'>{data?.category || "Category"}</p>
            ...
            <div className='flex items-center gap-2 text-2xl lg:text-3xl font-medium my-1'>
              <p className='text-red-600'>{displayINRCurrency(data.sellingPrice) || "Selling Price"}</p>
              <p className='text-slate-400 line-through'>{displayINRCurrency(data.price) || "Price"}</p>
            </div>
